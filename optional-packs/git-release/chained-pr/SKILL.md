...
## Skill Dependencies (orchestrator debe inyectar vía skill-resolver)

| Skill | Cuándo se necesita |
|-------|--------------------|
| `branch-pr` | Al crear cada PR de la cadena |
| `work-unit-commits` | Al planificar commits por slice |
| `senior-vcs-commander` | Si hay branching complejo o merge |

Orchestrator: seguir `_shared/skill-resolver.md` — resolver registry, inyectar `## Skills to load before work` con paths absolutos, verificar `skill_resolution`.
